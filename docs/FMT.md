...
## Decisions for the user, before code
...
4. **Inline blocks inside parentheses** — `map (|p|: if c: a else: b)` does not work today, because the layout treats the inside of a group as token soup until the matching `)`. That is a *layout* change, not a formatter one, and it is the first step of the formatter session, before `fmt` fixes columns around a form that would change. **Decided, in the user's terms:**
...
code. The user's words: "when the multiple arguments become too long together
...
its own** (the user, 2026-09-07 evening). Three links are vertical by the
...
the user's words: "put in evidence the method call nested in the closure."
