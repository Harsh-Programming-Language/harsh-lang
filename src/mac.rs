...
pub fn match_call<'a>(def: &'a Def, call: &[Token]) -> Result<(&'a Arm, Bindings), Error> {
    ...
    // A call written `m~ [ … ]` or `m~ { … }` delivers its arguments inside
    // that group, which is the call's delimiter rather than an argument.
    let call = match sole_group(call) {
        Some((lo, hi)) => &call[lo..hi],
        None => call,
    };
...
fn match_elems_in(
    ...
    for (n, e) in elems.iter().enumerate() {
        match e {
            ...
            Elem::Var { name, kind, .. } => {
                // A fragment ends at the next literal token the matcher
                // names -- `$k:expr => $v:expr` stops at the `=>` -- which is
                // Harsh's rule in place of Rust's follow-set table.
                let stop = elems[n + 1..].iter().find_map(|e| match e {
                    Elem::Lit(t) => Some(t.text.clone()),
                    _ => None,
                });
                let taken = take_fragment_until(*kind, call, *i, whole, stop.as_deref())?;
...
/// Does this call hand its arguments inside one bracket group -- `m~ [a; b]`,
/// `m~ { … }` -- as Rust's `vec![…]` does? Then the group is the call's own
/// delimiter and the matcher sees what is inside it.
fn sole_group(call: &[Token]) -> Option<(usize, usize)> {
    let first = call.first()?;
    if !matches!(first.kind, Tk::Open('[') | Tk::Open('{')) {
        return None;
    }
    let close = matching_close(call, 0)?;
    if close + 1 == call.len() {
        Some((1, close))
    } else {
        None
...
fn take_fragment(kind: Kind, call: &[Token], at: usize, whole: bool) -> Result<usize, Error> {
    take_fragment_until(kind, call, at, whole, None)
}
...
fn take_fragment_until(
    ...
    match kind {
        ...
        _ => {
            ...
            let mut d = 0i32;
            while k < call.len() {
                if d == 0 && stop.map_or(false, |s| call[k].text == s) {
                    break;
                }
                match call[k].kind {
                    Tk::Open(_) => d += 1,
...
fn spanning_repetition(
    lines: &[OutLine],
    at: usize,
) -> Option<(Vec<OutLine>, Option<Token>, usize)> {
    ...
    for k in at + 1..lines.len() {
        ...
        if close.first().map_or(false, |t| t.kind == Tk::Close(')')) {
            ...
            return Some((lines[at + 1..k].to_vec(), sep, k + 1));
        }
    }
    None
}
...
fn read_arms(toks: &[Token]) -> Result<Vec<Arm>, Error> {
    ...
    while i < toks.len() {
        ...
        // A matcher's own delimiters may be `(`, `[` or `{`, as in Rust: a
        // caller may use any of the three whatever the definition wrote.
        if !matches!(toks[i].kind, Tk::Open('(') | Tk::Open('[') | Tk::Open('{')) {
            ...
            if let Some(d) = defs.iter().position(|d| d.name == toks[i].text) {
                ...
                while end < toks.len() && toks[end].line_start.is_none() {
                    match toks[end].kind {
                        ...
                        // A `,` or `;` at this level belongs to whatever
                        // encloses the call -- a tuple, a list, an argument
                        // list -- not to the call, whose arguments juxtapose:
                        // `(twice~ 4, 0)` passes `4`, and `0` is the tuple's.
                        Tk::Comma | Tk::Semi if depth == 0 => break,
                        _ => {}
                    }
                    end += 1;
                }
                // A call may hand its arguments as a *block*: `m~ do:` with
                // the body indented beneath, or `m~\` with its entries. The
                // block is part of the call, as it is for a `!` macro.
                let opener = toks[..end]
                    .iter()
                    .rev()
                    .find(|t| !t.is_comment())
                    .map(|t| matches!(t.kind, Tk::Colon | Tk::Backslash))
                    .unwrap_or(false);
                if opener {
                    let col = line_col(toks, i);
                    while end < toks.len() {
                        match toks[end].line_start {
                            Some(c) if c <= col => break,
                            _ => end += 1,
                        }
                    }
                }
...
pub fn expand_all(toks: Vec<Token>, _taken: &[String]) -> Result<Vec<Token>, Error> {
    ...
    for pass in 0..limit {
        ...
        ctx += 1;
        let def = &defs[d];
        // The call's arguments. When they were handed as a block -- `m~ do:`
        // or `m~\` with the body beneath -- the opener belongs to the call,
        // not to the arguments, so it is dropped: what the matcher sees is
        // the block's own tokens.
        let mut call: Vec<Token> = out[at + 2..end].to_vec();
        loop {
            let head_len = call
                .iter()
            ...
            let opener = matches!(call[last].kind, Tk::Colon | Tk::Backslash) || call[last].is_kw("do");
            if !opener {
                break;
            }
            call.remove(last);
